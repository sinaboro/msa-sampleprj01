...
public class JwtAuthFilter implements GlobalFilter {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        try {
            ...
            //apigateway에서 member-service로 이동할 때
            //member-service에서는 token을 확인할 수 없다. 이유는 apigateway에서 확인을하고
            //member-service로 router 이동했기 때문에, 그런데, member-service에서 id, role등이
            //필요할수 있기 때문에 apigateway에서 routering할때 Header부분에 id, role를 추가해서
            //보낸다는 것이다.
